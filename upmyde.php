...
$username=$_POST['username'];
   $sql = "SELECT fname ,lname, adde,gender,bg,mno,username FROM regp where username=' ".$a. " ' ";
   mysql_select_db('new');
  ...
  "MOBILE NUMBER : {$row['mno']} <br> ".
 " EMAIL AS USERNAME : {$row['username']} <br> ".
         "<br>";
   }  
   mysql_close($conn);
?>
  <br><br>
<div class="login">
  <h2 class="login-header"> Update My Details</h2>
  <form  action="<?php echo ($_SERVER["PHP_SELF"]) ; ?>" method="post" class="login-container">
<center>   
  <p><input type="text" placeholder="Enter First Name"  value="<?php echo $row["fname"];  ?>" name="t1"  required ></p>
